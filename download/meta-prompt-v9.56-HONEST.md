...
Это нельзя менять. Любая редакция проверяется через hash.
...
## III. ПРЕ-ФИЛЛ ПРАВИЛА (G45)

- JSON → первый символ `{`. Без «Вот JSON:». Без code fence.
- YAML → первый ключ. Без `---`.
- Reasoning → в reasoning_content, НЕ в основном output.
