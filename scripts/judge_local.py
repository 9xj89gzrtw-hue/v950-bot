#!/usr/bin/env python3
"""
LLM-judge v3: использует локальную Qwen2.5-3B как primary judge.
Локальная модель стабильна, не обрезает ответы, нет rate limit.
"""
import json
import sys
import time
...
from multi_llm_v2 import MultiLLMv2, call_local, get_local_llm

VERSION = sys.argv[1] if len(sys.argv) > 1 else "v9.87-V2TEST"
...
ОТВЕТ — валидный JSON (без markdown):
...
def judge_with_local(task, output, meta_task=""):
    """Использует локальную Qwen2.5-3B как судью — стабильно, без rate limit."""
    user_msg = f"""TASK:
{task}
...
Оцени OUTPUT. Ответ — только JSON."""
    result = call_local(JUDGE_PROMPT, user_msg, max_tokens=800)
    return result
...
def judge_all():
    ...
    # Preload local model
    print("Preloading local Qwen2.5-3B as judge...")
    get_local_llm()
    print("Ready.\n")

    print("=" * 100)
    print(f"LOCAL JUDGE {VERSION}")
    print("=" * 100)
    ...
    for entry in data:
        ...
        print(f"\n▶ {test_id}")
        result = judge_with_local(entry.get("user_task", ""), entry["output"], entry.get("meta_task", ""))
